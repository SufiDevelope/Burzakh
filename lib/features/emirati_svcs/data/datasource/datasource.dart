...
abstract class EmiratiDataSource {
  ...
  Future<Either<String, Response>> updateCdaRequest(
      {required String caseId, required CdaModel? model});
  Future<Either<String, Response>> updateRtaRequest(
      {required String caseId, required RtaModel? model});
  ...
  Future<Either<String, Response>> getCaseName({required String userId});
}
...
class EmiratiFunctionClass extends EmiratiDataSource {
  ...
  Future<Either<String, Response>> cancelCda({
    ...
    try {
      ...
      var body = {"submission_id": id, "user_id": userId};
      Uri uri = Uri.parse(url);
  ...
  Future<Either<String, Response>> getCaseName({required String userId}) async {
    try {
      String url = "${AppApis.baseUrl}${AppApis.getCaseName}/$userId";
      Uri uri = Uri.parse(url);
      var response = await get(uri);
      return Right(response);
    } catch (e) {
  ...
  @override
  Future<Either<String, Response>> updateCdaRequest(
      {required String caseId, CdaModel? model}) async {
    try {
      String url = "${AppApis.baseUrl}${AppApis.updateCdaRequest}/$caseId";
      var body = {
        "user_id": model?.userId,
        "location_of_tent": model?.address,
      };
      Uri uri = Uri.parse(url);
      var response = await post(uri, body: body);
      return Right(response);
  ...
  @override
  Future<Either<String, Response>> updateRtaRequest(
      {required String caseId, RtaModel? model}) async {
    try {
      String url = "${AppApis.baseUrl}${AppApis.updateRtaRequest}/$caseId";
      var body = {
        "user_id": model?.userId,
        "location_of_house": model?.address,
      };
      Uri uri = Uri.parse(url);
      var response = await post(uri, body: body);
      return Right(response);
